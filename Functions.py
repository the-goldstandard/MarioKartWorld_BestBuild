...
def CoinCountProbabilities(race_or_KO):
    ...
    file=pyxl.load_workbook("Mario Kart World race data.xlsx",data_only=True)
    spreadsheet=file["Sheet1"]
    Lambda=spreadsheet.cell(row=2,column=16).value
    # The collection rate of coins (1/min)
    mu=spreadsheet.cell(row=3,column=16).value
    # The frequency of the player hit by an item and loses up to 3 coins each time (1/min)
    r=spreadsheet.cell(row=11,column=16).value*(3+0.5*race_or_KO)/3.5
